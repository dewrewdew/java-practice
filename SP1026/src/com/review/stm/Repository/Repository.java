...
public class Repository {
    ...
    private int count = 0;
    // 위 필드값은 초기값 설정을 위해 존재.
    // 멤버변수이므로 사실 프로그램이 실행되면서 다른값으로 업데이트 가능!
    // 애초에 list는 5짜리 리스트를 '가리키는 변수' 이므로 newlist 생성 후 그걸 가리키는 변수로 수정만 해주면 끝!
    ...
    public void save(StudentDto studentDto) {
            ...
            StudentDto[] newList = new StudentDto[newLength];
    ...
    //////////////////////작성중
    public void updateByName(String name, StudentDto studentDto) {
        for(int i = 0 ; i < count ; i++) {
            if(list[i].getName().equals(name)) {
                String updateName = studentDto.getName();
                int updateAge = studentDto.getAge();
                int updateScore = studentDto.getScore();
                StudentDto newStudentDto = new StudentDto(updateName, updateAge, updateScore);
                list[i] = newStudentDto;
            }
        }
    }
}
